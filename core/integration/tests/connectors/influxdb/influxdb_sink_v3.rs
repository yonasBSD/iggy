...
use crate::connectors::fixtures::InfluxDb3SinkFixture;
use bytes::Bytes;
...
#[iggy_harness(
    server(connectors_runtime(config_path = "tests/connectors/influxdb/sink_v3.toml")),
    seed = seeds::connector_stream
)]
async fn influxdb3_sink_writes_messages_to_db(
    harness: &TestHarness,
    fixture: InfluxDb3SinkFixture,
) {
    ...
    fixture
        ...
        .await
        .expect("Failed to wait for InfluxDB 3 points");
}
...
async fn influxdb3_sink_handles_bulk_messages(
    harness: &TestHarness,
    fixture: InfluxDb3SinkFixture,
) {
    let client = harness.root_client().await.unwrap();
...
async fn influxdb3_sink_payload_fields_stored_correctly(
    harness: &TestHarness,
    fixture: InfluxDb3SinkFixture,
) {
    let client = harness.root_client().await.unwrap();
...
async fn influxdb3_sink_large_batch(harness: &TestHarness, fixture: InfluxDb3SinkFixture) {
    let client = harness.root_client().await.unwrap();
    ...
    fixture
        ...
        .await
        .expect("Failed to wait for 500 InfluxDB 3 points");
}
...
async fn influxdb3_sink_recovers_backlogged_messages(
    harness: &TestHarness,
    fixture: InfluxDb3SinkFixture,
) {
    let client = harness.root_client().await.unwrap();
    ...
    fixture
        ...
        .await
        .expect("Failed to wait for 10 backlogged InfluxDB 3 points");
}

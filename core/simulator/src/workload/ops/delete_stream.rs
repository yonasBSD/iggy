...
//! `DeleteStream` op. Live stream name picked from shadow.
...
pub fn sample(
    ...
    match outcome {
        Outcome::Success => shadow.pick_stream_name(prng).map(|name| Input { name }),
    }
}
...
pub fn predicted_effect(input: &Input, outcome: Outcome) -> Effect {
    match outcome {
        Outcome::Success => Effect::RemoveStream {

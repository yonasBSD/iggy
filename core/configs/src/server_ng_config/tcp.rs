...
//! Server-ng TCP listener schema.
//!
//! Field shape mirrors the legacy [`crate::tcp::TcpConfig`] verbatim;
//! the type is forked into `server_ng_config` so server-ng can evolve
//! its TCP/TLS surface (additional knobs, removed knobs) independently
//! of the legacy server. No semantic change at fork time.

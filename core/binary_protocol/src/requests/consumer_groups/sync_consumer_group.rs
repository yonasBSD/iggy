...
/// `SyncConsumerGroup` request.
///
/// A member asks the coordinator for its current partition assignment (and the
/// group generation) so it can select partitions to poll client-side.
///
...
impl WireEncode for SyncConsumerGroupRequest {
...
impl WireDecode for SyncConsumerGroupRequest {
...
mod tests {
    ...
    fn roundtrip_named() {
        ...
        let (decoded, consumed) = SyncConsumerGroupRequest::decode(&bytes).unwrap();
    ...
    fn truncated_returns_error() {
        ...
        for i in 0..bytes.len() {
            assert!(
                SyncConsumerGroupRequest::decode(&bytes[..i]).is_err(),

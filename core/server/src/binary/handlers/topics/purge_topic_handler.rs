...
use iggy_binary_protocol::requests::topics::PurgeTopicRequest;
use iggy_common::purge_topic::PurgeTopic;
use iggy_common::{IggyError, SenderKind};
use std::rc::Rc;
use tracing::{debug, instrument};

#[instrument(skip_all, name = "trace_purge_topic", fields(iggy_user_id = session.get_user_id(), iggy_client_id = session.client_id))]
pub async fn handle_purge_topic(
    req: PurgeTopicRequest,
    ...
    debug!(
        "session: {session}, command: purge_topic, stream_id: {stream_id}, topic_id: {topic_id}"
    );

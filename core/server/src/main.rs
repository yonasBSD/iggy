...
* regarding copyright ownership.  The ASF licenses this file
   ...
   let active_sessions = Box::new(DashMap::new());
   let active_sessions = Box::leak(active_sessions);
   let active_sessions: EternalPtr<DashMap<u32, std::sync::Arc<Session>>> = active_sessions.into();

   streams.with_components(|components| {under the Apache License, Version 2.0 (the
